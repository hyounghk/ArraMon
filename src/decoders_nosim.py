...
class ActionDecoder(nn.Module):
    ...
    def forward(self, epoch, mapids, turn, insts, insts_len, gt_acts, gt_path, gt_rot, phase='navi'):
        ...
        act_seq = gt_acts.new_zeros((batch_size, seq_len-1), dtype=torch.long).cuda()
        pos_seq = gt_acts.new_zeros(batch_size, seq_len, 3).float().cuda()
        pos_seq_gt = gt_acts.new_zeros(batch_size, seq_len, 3).float().cuda()
        pos_len = gt_acts.new_ones((batch_size), dtype=torch.long).cuda() * seq_len
        seqLogprobs = gt_acts.new_zeros(batch_size, seq_len-1).float().cuda()
        logits = gt_acts.new_zeros(batch_size, seq_len-1, self.nActions).float().cuda()
        placed_obj_pos = gt_acts.new_zeros(batch_size, 3).float().cuda() - 1
        gt_track = gt_acts.new_zeros((batch_size), dtype=torch.long).cuda()
        pos_track = gt_acts.new_zeros((batch_size), dtype=torch.long).cuda() - 1

        if not self.training and phase == 'assembly':
        ...
        for t in range(seq_len):
            ...
            if self.training:
                if phase == 'navi':
                    imgtag = "navi_" + str(turn) + '_' + str(t)
                else:
                    imgtag = "assem_" + str(turn) + '_' + str(t)

            if self.training:
                pick_ids, place_pos, agent_pos, agent_rot, vis_feat = self.sim.getFeats(batch_size, imgtag)
            else:
        ...
        pos_track += 1
        if self.training:
            pick_ids_k, _, agent_pos_k, agent_rot_k, vis_feat_k = self.sim.getFeats(batch_size, imgtag)
        else:
            pick_ids_k, _, agent_pos_k, agent_rot_k, vis_feat_k = self.sim.sendActionK(batch_size, record, imgtag)

        return seqLogprobs, logits, act_seq, pos_seq, pos_track, pos_seq_gt, pick_ids, placed_obj_pos, agent_pos.cuda()
